...
const char SamplingPeriod[22][8] = {{"33.333mS"}, {"16.667mS"}, {"6.6667mS"}, {"3.3333mS"}, {"1.6667mS"}, {"666.67uS"}, {"333.33uS"}, {"166.67uS"}, {"66.667uS"}, {"33.333uS"}, {"16.667uS"}, {"6.6667uS"}, {"3.3333uS"}, {"1.6667uS"}, {"666.67nS"}, {"333.33nS"}, {"166.67nS"}, {"69.444nS"}, {"41.667nS"}, {"27.778nS"}, {"13.889nS"}, {"13.889nS"}};

const char SubSamplingPeriod[13][8] = {{"3.3333mS"}, {"6.6667mS"}, {"16.667mS"}, {"33.333mS"}, {"66.667mS"}, {"166.7mS"}, {"333.33mS"}, {"500.00mS"}, {"1.0000 S"}, {"2.0000 S"}, {"4.0000 S"}, {"10.000 S"}, {"20.000 S"}};
...
u8 InitFileSystem()
{
  ...
  if (SectorSize > LARGEST_SECTOR_SIZE)
  ...
  { //root dir starting/ending addresses
...
u8 Color_Num(u16 Color) //for 16 color BMP's
{
  ...
  else if ((Color == CYAN) || (Color == 0xEF60) || (Color == 0xF7A0))
    return 1;
  //else if(Color== YEL   ) return 3;
  else if ((Color == YEL) || (Color == 0x077D) || (Color == 0x07BE))
    return 3;
  else if (Color == PURPL)
    return 5;
  else if (Color == GRN)
  ...
  else if ((Color == CYAN_) || (Color == 0xDEE0))
    return 2; //include alt fast dims
              //else if(Color== YEL_  ) return 4;
  else if ((Color == YEL_) || (Color == 0x06FB))
    return 4;
  else if (Color == PURPL_)
    return 6;
  else if (Color == GRN_)
    return 8;
  else if (Color == GRAY)
  ...
  //else                              return 13;
    ...
    return 13; //add black as viable color so background saves properly
  else if (Color == 0x87F0)
    return 14; //bright green used in serial decoding hex values and arrows added
  else if (Color == 0x7BEF)
    return 15; //dark gray for i2c arrows
  else
    return 9; //anything else would be grid background mixed with something so show grid, not black holes
}
...
u8 Save_Img(void)
{
  ...
  u32 pDirAddr[1];
  if (InitFileSystem() != 0)
    return 10; //does not use make_filename so may not be initiated

  if (__OpenFileWr(SecBuff, "ROM_IMG BIN", pCluster, pDirAddr) != OK)
    return DISK_ERR;
  for (i = 0; i < 0x80000; i++)
  { //SecBuff here will be either 512 or 4096
    SecBuff[j] = *(char *)(0x8000000 + i);
    ChkSum ^= SecBuff[j];
    if (++j >= SectorSize)
    {
      if (WriteSector(k++, (u16 *)pCluster, (u32 *)pDirAddr))
        return WR_ERR;
      j = 0;
    }
  }
  if (__CloseFile(SecBuff, 0x80000, pCluster, pDirAddr) != OK)
    return WR_ERR;
  ...
  for (i = 0; i < 0x80000; i++)
  {
    ...
    {
      j = 0;
      if (__ReadFileSec(SecBuff, pCluster) != OK)
        return RD_ERR;
...
u8 Load_Dat(u8 FileNum)
{
  ...
  {
    for (i = 0; i < 8; i++)
      Filename[i] = SelectedFileName[i];
  }
  else
  {
    if (Make_Filename(FileNum, Filename) != 0)
      return 1;
  }
...
u8 Save_Dat(u8 FileNum)
{
  ...
  if (flash_mode == FLASH_8M)
  {
    ...
    {
      for (i = 0; i < 397; i++)
        SecBuff[i + j * 512] = TrackBuff[i * 4 + j];
      SecBuff[397 + j * 512] = Title[j][POSI].Value;
    }
    if (WriteSector(j, (u16 *)pCluster, (u32 *)pDirAddr))
      return WR_ERR;
  }
  else
  {
  ...
  if (__CloseFile(SecBuff, 0x0800, pCluster, pDirAddr) != OK)
    return WR_ERR;
  return OK;
}
/*******************************************************************************
...
u8 Save_Bmp(u8 FileNum)
{
  ...
  if (__OpenFileRd(SecBuff, Filename, pCluster, pDirAddr) == 0)
  { // original file exists
    if (OverWriteWarn())
      return 200;
    if (DeleteFile(SecBuff, Filename))
      return WR_ERR;
  }
  ...
  if ((_4_source == 10) || (_4_source == 11))
  {                                 //only save in 64K color if displaying spectrograph or map
  ...
  if (__CloseFile(SecBuff, FileSize, pCluster, pDirAddr) != OK)
    return WR_ERR; //2nd var just seems to be size stamp, alterbios
...
u8 Save_Buf(u8 FileNum)
{
  ...
  if (OSBufferLogic())
  { //store OS data in unused part of DataBuf
    for (i = 0; i < 2; i++)
    {
      for (j = 512; j < 1024; j++)
      {
  ...
  { // < 16384/   512 or 4096   =    32 or 4
    memcpy(SecBuff, &(DataBuf[i * SectorSize / 4]), SectorSize);
    if (WriteSector(i, (u16 *)pCluster, (u32 *)pDirAddr))
      return WR_ERR;
  }
  memset(SecBuff, 0, 512);
  p = (u16 *)SecBuff;
  for (i = 0; i < 4; i++)
  { // save each corresponding value in the display menu
    *p++ = Title[i][0].Value;
    *p++ = Title[i][1].Value;
    *p++ = Title[i][2].Value;
    *p++ = Title[i][3].Value;
  ...
  *p++ = ChartMode;
  ...
  if (j)
    return j;
  if (__CloseFile(SecBuff, 0x4200, pCluster, pDirAddr) != OK)
    return WR_ERR;
  return 0;
}
...
u8 WriteFileSec(u8 *SecBuff, u16 *pCluster, u32 *pDirAddr)
{
  if (((flash_mode == FLASH_8M) && (*pCluster > 2040)) || //abort file write if at end of drive space
      ((flash_mode == FLASH_2M) && (*pCluster > 4031)))
  {                                              //Alterbios does not seem to return much of anything,
  ...
  { //least doesn't loop around to destroy the FAT
    if (__ProgFileSec(SecBuff, pCluster) != OK)
      return WR_ERR;
  }
  return 0;
}
...
u8 Load_Arbt(u8 FileNum)
{
  ...
  while (*p != 44)
  {
    if ((*p > 47) && (*p < 58))
      ...
      p++; //retrieve "header" containing number of samples to be used
    Start++;
    if ((j > 3) || (Start > 200))
      return 1; //return ERR if samples >3 digits or comma not found after 200 chars
  ...
  while (Sample < 720)
  {
    ...
    for (i = Start; i < SectorSize; i++)
    {
      ...
      if (Tmp == 44)
      { //once delimiter found, convert and load value
        ATT_DATA[Sample] = AsciiToU16(ParseStr, j);
        if (ATT_DATA[Sample] < 0)
          ATT_DATA[Sample] = 0;
        if (ATT_DATA[Sample] > 4095)
          ATT_DATA[Sample] = 4095; //clip if out of range
        j = 0;
        Sample++; //reset string position counter and advance sample position
      }
      else
      { //start loading string here
        if ((Tmp > 47) && (Tmp < 58))
          ParseStr[j++] = Tmp; //eliminate white space or other non relevant chars
        if (j > 4)
          j = 4; //limit to 5 valid characters
      }

      if (Sample > (ArbtSampleNumber - 1))
        goto EndOfFile; //if sample goes to ArbtSampleNmber, is now past base 0 sample number index
...
u8 ReadDir(char *ext)
{                          //Load 15 filenames matching extension in Label, starting with match # DirRange
  ...
  for (*pDirAddr = RootStart; *pDirAddr <= RootStop;)
  {                                                    //scan for filenames in root directory
    ...
    {
      if (pBuffer[n] == 0)
        return LabelIndex;                                         //no more entries, partially filled list page
      if ((pBuffer[n + 8] == ext[0]) && (pBuffer[n + 9] == ext[1]) //look for file extension match
          && (pBuffer[n + 10] == ext[2]) && (pBuffer[n] != 0xE5))
      { //and exclude deleted files
        if (EntryCounter >= DirRange)
        { //start offset for next page
          for (i = 0; i < 8; i++)
            Label[LabelIndex][i] = pBuffer[n + i]; //copy first 8 chars of filename
          Label[LabelIndex][8] = 0x2E;             //"."
          for (i = 0; i < 3; i++)
            Label[LabelIndex][9 + i] = ext[i]; //extension
          LabelIndex++;
        }
        EntryCounter++;
      }
      if (LabelIndex >= 15)
        return LabelIndex; //page is full
...
u8 DeleteFile(u8 *pBuffer, char *FileName)
{
  ...
  for (*pDirAddr = RootStart; *pDirAddr <= RootStop;)
  { //scan for filename in root directory
    if (__ReadDiskData(pBuffer, *pDirAddr, End) != OK)
      return RD_ERR;
    for (n = 0; n < End; n += 32)
    {
      for (i = 0; i < 11; i++)
...
FileNameFound:
  ...
  if (__ProgDiskPage(pBuffer, *pDirAddr) != 0)
    return WR_ERR; //write section back to disk (4096 bytes for 8M, 256 for 2M)

  if (__ReadDiskData(pBuffer, FatStart, DirPageSize) != OK)
    return RD_ERR; //2MB is 0x1800 long
  if (*pCluster < 2)
    return RD_ERR;

  do
  {                                       //Follow the white rabbit, but don't get lost, or your files will end up cross-linked!
    ...
    if (flash_mode == FLASH_8M)
    {
      if (Offset >= 3072)
        return RD_ERR;
      if (i++ > 2048)
        return RD_ERR; //all good things MUST come to an end
    }
    else
    {
      if (Offset >= 6144)
  ...
  {
    if (__ProgDiskPage(pBuffer + (i * 256), FatStart + (i * 256)) != 0)
      return WR_ERR; //write section back to disk
  }

  if (flash_mode == FLASH_8M)
  { //synch FAT2 with FAT1
    if (__ReadDiskData(pBuffer, 0x1000, 0x1000) != OK)
      return RD_ERR;
    if (__ProgDiskPage(pBuffer, 0x2000) != OK)
      return WR_ERR;
  }
  else
  {
    for (i = 0; i < 24; i++)
    {
      if (__ReadDiskData(pBuffer, 0x1000 + (i * 256), 256) != OK)
        return RD_ERR;
      if (__ProgDiskPage(pBuffer, 0x2800 + (i * 256)) != OK)
        return WR_ERR;
    }
  }
  if (*pCluster < 0xFF0)
...
void FileMessage(u8 i)
{
  ...
  if ((_Curr[2].Value == BUF) && (_Curr[0].Value == LOAD))
    Delayms(300);
  else
    Delayms(900);
  _Curr[1].Flag |= UPDAT;
...
u8 Load_Uart(u8 FileNum)
{
  ...
  if (FileEntryAdd(pBuffer, pDirAddr, Filename) != OK)
    return RD_ERR; //retrieve filename dir entry address

  if (__ReadDiskData(pBuffer, *pDirAddr + 28, 4) != OK)
    return RD_ERR; //read file size value at offset 28 of FAT12 directory entry
  for (n = 0; n < 4; n++)
    UartFileSize += (u32)pBuffer[n] << (n * 8); //assemble u32 value from 4 bytes
  if (UartFileSize > 4096)
    UartFileSize = 4096; //limit to size of SecBuff
  //if(UartFileSize>0x1800)UartFileSize=0x1800;                        //limit to size of SecBuff
  n = 0;
  while (n < UartFileSize)
  {
    if (__ReadFileSec(&SecBuff[n], pCluster) != OK)
      return RD_ERR;
    n += SectorSize;
  }
...
u8 FileEntryAdd(u8 *pBuffer, u32 *pDirAddr, char *Filename)
{ //writes directory filename entry address in pDirAddr
  u16 n;
  u8 i;
  for (*pDirAddr = RootStart; *pDirAddr <= RootStop;)
  { //scan for filename in root directory
    if (__ReadDiskData(pBuffer, *pDirAddr, 256) != OK)
      return RD_ERR;
    for (n = 0; n < 256; n += 32)
    {
      for (i = 0; i < 11; i++)
...
u8 Load_Buf(u8 FileNum)
{
  ...
  if (Title[3][0].Value < 11)
  { //set proper menu sub items for ch 4
    if (Detail[3] == 2)
      Detail[3]++;
  }
  else
...
void save_parameter(void)
{
  ...
  if (TempPar[0] != 0xAA55)
  {              //do not save temp parameters again if already saved, indicates original parms not restored
...
u8 CSVdataModeLogic(void)
{
  return ((((OSBuffer == 0) || (_T_base > 16)) && (DigChLockout == 0) && ((!ChartLogic()) || (ChartMode == 0))) && ((_3_source) || (_4_source == 1) || (_4_source == 4) || (_4_source == 5)));
}
...
s32 CSVdata(u8 Ch, u8 Inv, u16 i, u8 data)
{
  ...
  p1[Ch] += range;
  temp = *p1[Ch] + ((data - ADCoffset) - Title[Ch][POSI].Value);
  p2[Ch] += range;
  if (CalFlag > 0)
    temp = (*p2[Ch] * temp) / 1024;
  temp *= Y_Attr[range].SCALE;
  if (Title[Ch][SOURCE].Value > 2)
    temp *= 10;
  if (Inv)
    temp = -temp;
  return temp;
...
u8 Save_Csv(u8 FileNum)
{
  ...
  { // original file exists
    if ((AutoSaveBuf == 0) || (ChartLogic() == 0))
    { // overwrite file unconditionally in chart auto save mode
      if (OverWriteWarn())
        return 200;
    }
    if (DeleteFile(SecBuff, Filename))
  ...
  memcpy(&SecBuff[k], SPeriod, 7);
  k += 7;
  if (ChartLogic())
  {
    memcpy(&SecBuff[k], SubSamplingPeriod[SubIndex - 1], 8);
    k += 8;
  }
  else
  {
    memcpy(&SecBuff[k], SamplingPeriod[_T_base], 8);
    k += 8;
  }
  SecBuff[k++] = 0x2C;
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    if (CSVdataModeLogic())
    {
      memcpy(&SecBuff[k], Offset, 4);
      k += 4;
      u8ToDec3(Val, Title[i][POSI].Value, 0);
      memcpy(&SecBuff[k], Val, 3);
      k += 3;
    }
    if (i < 2)
    {
      if (CSVdataModeLogic())
      {
        memcpy(&SecBuff[k], DigSampling[i], 7);
        k += 7;
        if (Title[i][0].Value > 2)
        {
          memcpy(&SecBuff[k], AmplitudeX10Steps[Title[i][RANGE].Value], 6);
          k += 6;
        }
        else
        {
          memcpy(&SecBuff[k], AmplitudeSteps[Title[i][RANGE].Value], 6);
          k += 6;
        }
      }
      else if (((OSBuffer) && (_T_base < 17)) || ((ChartLogic()) && (ChartMode)))
      {
        memcpy(&SecBuff[k], OSsampling[i], 13);
        k += 13;
      }
      else
      {
        memcpy(&SecBuff[k], Sampling[i], 13);
        k += 13;
      }
    }
    else
    {
      if (CSVdataModeLogic())
      {
        memcpy(&SecBuff[k], DigSampling[i], 10);
        k += 10;
      }
      else if (((OSBuffer) && (_T_base < 17)) || ((ChartLogic()) && (ChartMode)))
      {
  ...
  for (i = (discard + h); i < bag_max_buf; i++)
  {
    ...
    { //HOURS
      for (j = 0; j < 4; j++)
      {
        ...
        if (k >= length)
        {
          if (WriteSector(n++, (u16 *)pCluster, (u32 *)pDirAddr))
            return WR_ERR;
          k = 0;
        }
      }
    ...
    if (CSVdataModeLogic())
    { //direct buffer value copy mode w/digital ch's
      ...
      if ((DataBuf[i] & 0x010000) != 0)
        temp += 20;
      u8ToDec3(Num[2], temp, 0);
      temp = Title[TRACK4][POSI].Value;
      if ((DataBuf[i] & 0x020000) != 0)
        temp += 20;
      u8ToDec3(Num[3], temp, 0);
    }
    else
    {                                                                   //scaled voltage display mode without digital ch's
    ...
    for (j = 0; j < 4; j++)
    {
      if ((j > 1) && (!((CSVdataModeLogic()) || (((OSBuffer) && (_T_base < 17)) || ((ChartLogic()) && (ChartMode))))))
      {                      //do not export digital chs if not enabled
  ...
  if (__CloseFile(SecBuff, (n * length) + k, pCluster, pDirAddr) != OK)
    return WR_ERR;

  if (flash_mode == FLASH_2M)
  { //insert correct file length
    if (FileEntryAdd(SecBuff, pDirAddr, Filename) != 0)
      return RD_ERR;
    if (__ReadDiskData(SecBuff, *pDirAddr & 0xFFFFFF00, 256) != OK)
      return RD_ERR;
    *(u32 *)(SecBuff + (*pDirAddr & 0xFF) + 0x1C) = (n * length) + k;
    if (__ProgDiskPage(SecBuff, *pDirAddr & 0xFFFFFF00) != 0)
      return WR_ERR;
  }
...
u8 Load_Param(u8 FileNum)
{
  ...
  {
    for (i = 0; i < 8; i++)
      ConfigFileName[i] = Filename[i] = SelectedFileName[i];
  ...
  {          // do not load correction factors if not loading boot config
  ...
  if (BootLoad)
  { //"system" variable, only load on boot
    Sum = *p;
    if ((*p++ >> 8) == 0xAB)
      ADCoffset = Sum;
    else
      ADCoffset = 54; //0xAB "key" assures default of 54 if ADCoffset value not originally saved
  }
  else
    p++;
  ...
  if (BootLoad)
    ...
    p++; //"system" variable, only load on boot
         /*
    FreeRunEnable=   *p++;
    FPGAosFlag=      *p++;
...
*/
  ...
  if (BootLoad)
    ...
    p++;
  if (BootLoad)
    DisableCursorDisplay = *p++;
  else
    p++;

  if ((GenUartMode < 1) || (GenUartMode > 6))
    GenUartMode = 3; //default to 8N
  if ((GenBaudIndex < 16) || (GenBaudIndex > 15871))
    GenBaudIndex = 7500;
  if (SpiNumBits < -145)
    SpiNumBits = 8; //these would flag read errors from possible corrupt config files
  if (SpiNumBits > 192)
    SpiNumBits = 8; //in such a case return to defaults
  if (SpiAdj > 3)
    SpiAdj = 0;
  if (SpiBitOrder > 1)
    SpiBitOrder = 0;
  ...
  if (LoBeepLevel > 9)
    LoBeepLevel = 0;
  if (DisableCursorDisplay > 3)
    DisableCursorDisplay = 0;
  return OK;
}
/*******************************************************************************
...
void ProcessConfigName(void)
{
  ...
  for (j = 7; j >= 0; j--)
  { //remove trailing spaces and underscores for config file display
    if ((ConfigFileName[j] == 32) || (ConfigFileName[j] == 95))
      ConfigFileName[j] = 0;
    else
      break;
  }
  for (i = 0; i < 8; i++)
    if (ConfigFileName[i] == 95)
      ConfigFileName[i] = 32; //change all other underscores to spaces for vertical display
  for (i = 0; i < 8; i++)
  {
    switch (ConfigFileName[i])
    ...
    case 64: //@
      ...
      break;
    default:
      if (((ConfigFileName[i] > 32) && (ConfigFileName[i] < 37)) || (ConfigFileName[i] > 124))
        ConfigFileName[i] = 63; //limit to char array bounds
    }
  }
}
